...
def get_available_bedrock_port(config: dict, guild_id: int) -> int | None:
    """
    Retourne le premier port Bedrock (UDP) disponible dans la plage 19132-19200 pour la guild.
    Retourne None si tous les ports sont utilisés.
    """
    guild_str = str(guild_id)
    used: set[int] = set()
...
def assign_bedrock_port(config: dict, guild_id: int) -> int:
    """
    Retourne le premier port Bedrock disponible.
    Lève ValueError si aucun port n'est disponible dans la plage.
    """
    port = get_available_bedrock_port(config, guild_id)
    if port is None:
        raise ValueError(
